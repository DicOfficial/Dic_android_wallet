...
public class DepositFragment extends BaseFragment {
    ...
    @BindView(R.id.tv_address_pubkey)
    TextView tvAddressPubkey;
    @BindView(R.id.tv_tap_to_copy_pubkey)
    TextView tvTapToCopyPubkey;
    ...
    @OnClick({R.id.tv_address_pubkey,R.id.tv_tap_to_copy_pubkey})
    public void onClick1(View view) {

        String s = walletManager.anyToWif(walletManager.getPrivateKey());
        ECKey ecKey = ECKey.fromPrivate(s.getBytes());
        String publicKeyAsHex = ecKey.getPublicKeyAsHex();
        ClipboardUtils.copyToClipBoard(getContext(), publicKeyAsHex);
    }
    ...
    protected void init() {
        ...
        if (walletManager.getWalletAddressForDeposit() != null && !walletManager.getWalletAddressForDeposit().isEmpty()) {
            ...
            ECKey ecKey = ECKey.fromPrivate(s.getBytes());
            String publicKeyAsHex = ecKey.getPublicKeyAsHex();
            Log.d("pubkey", publicKeyAsHex);
            tvAddressPubkey.setText(publicKeyAsHex);
        }
    }
