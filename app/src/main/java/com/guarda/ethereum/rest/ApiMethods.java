...
//        return getBaseApi(KmdExplorer.KMD_TEST_EXPLORER_BASE_URL).create(KmdApi.class);
    ...
    static KmdApi createKmdApi01() {
        return getBaseApi(KmdExplorer.KMD_EXPLORER_BASE_URL_01).create(KmdApi.class);
//        return getBaseApi(KmdExplorer.KMD_TEST_EXPLORER_BASE_URL).create(KmdApi.class);
    }
    ...
    private static Retrofit getBaseApi(String baseUrl) {
        ...
        try {
            response = client11.newCall(request).execute();
            if (response.isSuccessful()){
                url = KmdExplorer.KMD_EXPLORER_BASE_URL;
            } else {
                url = KmdExplorer.KMD_EXPLORER_BASE_URL;
            }
        } catch (IOException e) {
            url = KmdExplorer.KMD_EXPLORER_BASE_URL_01;
        }*/
        /*Runtime runtime = Runtime.getRuntime();
        try {
            Process p = runtime.exec("ping -c 3 "+KmdExplorer.KMD_EXPLORER_BASE_URL_01);
            int ret = p.waitFor();
            Log.i("Avalible", String.valueOf(ret));
        } catch (Exception e) {
            e.printStackTrace();
        }*/
...
//                                     Log.d("svcom ", "clearText body " + response.errorBody().string());
    ...
    static void makeRequest1(String address, Call call, @Nullable final RequestListener listener) {
        call.enqueue(new Callback<Object>() {
                         @Override
                         public void onResponse(Call<Object> call, Response<Object> response) {
                             if (response.isSuccessful()) {
...
//                                     Log.d("svcom ", "clearText body " + response.errorBody().string());
                         ...
                         @Override
                         public void onFailure(Call<Object> call, Throwable t) {
                             RequestorBtc.getBalanceKmdNew1(address, listener);
                             Log.d("svcom", "failure " + t.getMessage() + " " + t.toString());
                             if (listener != null) {
                                 listener.onFailure(t.getMessage() + " " + t.toString());
                             }
    ...
    static void makeRequest2(String address,int from, int to, Call call, @Nullable final RequestListener listener) {
        call.enqueue(new Callback<Object>() {
                         @Override
                         public void onResponse(Call<Object> call, Response<Object> response) {
                             if (response.isSuccessful()) {
...
//                                     Log.d("svcom ", "clearText body " + response.errorBody().string());
                         ...
                         @Override
                         public void onFailure(Call<Object> call, Throwable t) {
                             RequestorBtc.getTransactionsKmdNew1(address, from, to, listener);
                             Log.d("svcom", "failure " + t.getMessage() + " " + t.toString());
                             if (listener != null) {
                                 listener.onFailure(t.getMessage() + " " + t.toString());
                             }
    ...
    static void makeRequest3(String address,Call call, @Nullable final RequestListener listener) {
        call.enqueue(new Callback<Object>() {
                         @Override
                         public void onResponse(Call<Object> call, Response<Object> response) {
                             if (response.isSuccessful()) {
...
//                                     Log.d("svcom ", "clearText body " + response.errorBody().string());
                         ...
                         @Override
                         public void onFailure(Call<Object> call, Throwable t) {
                             RequestorBtc.getUTXOListKmdNew1(address,  listener);
                             Log.d("svcom", "failure " + t.getMessage() + " " + t.toString());
                             if (listener != null) {
                                 listener.onFailure(t.getMessage() + " " + t.toString());
                             }
